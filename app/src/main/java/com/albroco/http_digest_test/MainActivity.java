...
import com.albroco.androidhttpdigest.AuthorizationRequestHeader;
...
public class MainActivity extends AppCompatActivity {
  ...
  private static class MakeRequestAsyncTask extends AsyncTask<Void, Void, Void> {
    ...
    private void makeRequest(HttpDigestState httpDigestState) throws IOException {
      HttpURLConnection connection;

      connection = (HttpURLConnection) new URL(TEST_URL).openConnection();
      httpDigestState.setHeadersOnRequest(connection);
      httpDigestState.updateWithResponse(connection);
      Log.i(LOG_TAG,
          "Header out: " +
